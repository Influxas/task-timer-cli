...
When multiple functions manipulate the same thing:

file
taskName
startTime

that thing usually becomes a class.

Right now:

startTimer → writes state
getTime → reads state
stopTimer → deletes state

All three operate on timer state.

So the developer thinks:

This should become a TimerState class.

What the class will represent
...
And the file becomes just the storage mechanism.
